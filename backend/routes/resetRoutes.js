...
const transporter = nodemailer.createTransport({
  ...
  router.post('/forgot-password', async (req, res) => {
    ...
    try {
      ...
      const resetPasswordUrl = `http://localhost:5173/reset-pass/${token}`;
      await transporter.sendMail({
        from: 'achintham63@gmail.com',
        to: email,
        subject: 'Password Reset Link',
        html: `<p>Please use the following link to reset your password: <a href="${resetPasswordUrl}">Reset Password</a></p>`
      });
